...
// JankyBorders uses (lower latency, draws above every layer); it is reimplemented from scratch
// here — no GPL code is involved (see the license discussion).
//
// Every private symbol is resolved at runtime with dlsym from SkyLight.framework. If ANY symbol
// is missing the initializer returns nil and FocusBorderController falls back to the public
// OverlayBorderRenderer — so a future macOS that drops/renames a symbol degrades gracefully
// instead of failing to link or crashing.
//
// Coordinates: SkyLight window origins are global top-left — the same space as ZTRect — so no
// NS flip is needed. The per-window CGContext is bottom-left (standard CG), handled in draw().

import AppKit
import ZTCore

final class SkyLightBorderRenderer: BorderRenderer {
    // MARK: private SkyLight signatures
    ...
    init?() {
        ...
        orderWindow = ow; moveWindow = mw; setShape = ss; contextCreate = cc; flush = fl
        cid = mainConn()
    ...
    func render(frame: ZTRect?, style: BorderStyle) {
        ...
        // Position (global top-left) — no flip.
        var origin = CGPoint(x: outer.origin.x, y: outer.origin.y)
    ...
    private func ensureWindow(size newSize: CGSize) -> Bool {
        ...
        if wid == 0 {
            var newID: UInt32 = 0
            // type 2 = buffered; origin set later via moveWindow.
            guard newWindow(cid, 2, 0, 0, region, &newID) == 0, newID != 0 else { return false }
            wid = newID
    ...
    private func draw(contentSize: CGSize, windowFrame frame: ZTRect, pad: Double, style: BorderStyle) {
        ...
        // The window frame sits inside the padded content, inset by `inset`; grown for the stroke.
...
private extension CGRect {
    ...
    init(x: Double, y: Double, w0: Double, h0: Double) {
        self.init(x: x, y: y, width: w0, height: h0)
    }
}
