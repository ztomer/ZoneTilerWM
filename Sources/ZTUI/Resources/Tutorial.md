...
ZoneTilerWM learns where you put each app and reuses it. The menubar **Window Analytics** window shows a heatmap of where windows land, per-app footprints, a daily-activity trend, and the full learned table.

## Settings & config

- The menubar **Settings…** window edits everything (General, Keys, Apps, Layouts, Pomodoro, Advanced) and writes changes live.
- The config file is `~/.config/ZoneTilerWM/config.toml`. Editing it by hand live-reloads too. If two actions share a shortcut, the agent logs a warning and the **Keys** tab shows a conflict banner.
