...
-- Window cache — consume cached state (app_name, screen_id, isStandard, isMinimized)
-- instead of re-querying the Accessibility API on every zone focus press. AX calls
-- are intercepted by SentinelOne EDR and cost ~1ms each; avoiding ~4N of them per
-- press (where N is the number of open windows) materially speeds up cycling.
...
local function collect_zone_windows(monitor_id, zone_key, screen_obj, zone_tiles_for_this_zone)
    ...
    -- Pull candidates from the cache (already filtered to isStandard && !isMinimized
    -- && on the target screen). Each entry is an info table with a cached `app_name`
    -- and the live `hs.window` reference, so we can skip per-window AX calls for
    -- isStandard/isMinimized/screen:id()/application:name().
    ...
    -- Pre-compute Z-order map to avoid O(N^2) complexity and redundant C-API calls.
    -- orderedWindows() is a single bulk AX call; we keep it but limit per-window work.
    local z_order_map = {}
    ...
    for _, info in ipairs(windows_on_screen) do
        ...
        if pos and pos.monitor_id == monitor_id and pos.zone_key == zone_key then
            table.insert(zone_windows_collected, {
                window = info.window,
                window_id = win_id,
                app_name = info.app_name, -- cached, no AX call
    ...
    -- We still call win:frame() directly here: user-driven drags don't invalidate
    -- the cached frame, and the overlap calculation requires an up-to-date rect.
    for _, info in ipairs(windows_on_screen) do
...
function focus_manager.cycle_windows_in_zone(focused_window_before_call, target_zone_key)
    ...
    local current_focus_cycle_manager = current_focus_cycle_manager -- shorthand

    -- Determine if the cycle needs to be rebuilt
    local needs_rebuild = false
    if target_zone_key ~= current_focus_cycle_manager.zone_key or current_monitor_id ~= current_focus_cycle_manager.monitor_id_logical or #current_focus_cycle_manager.window_ids_in_order ==
        0 then
        needs_rebuild = true
        debug_log(string.format(
            ...
            target_zone_key, current_focus_cycle_manager.zone_key or "nil", current_monitor_id, current_focus_cycle_manager.monitor_id_logical or "nil",
        ...
        if not zone_tiles_for_check or #zone_tiles_for_check == 0 then
            ...
            if #actual_windows_in_zone_now ~= #current_focus_cycle_manager.window_ids_in_order then
                ...
                debug_log(string.format("Rebuilding cycle: Number of windows in zone changed. Stored: %d, Actual: %d.",
                    #current_focus_cycle_manager.window_ids_in_order, #actual_windows_in_zone_now))
            else
    ...
    if needs_rebuild then
        ...
        current_focus_cycle_manager.window_ids_in_order = {}
        current_focus_cycle_manager.current_idx_in_cycle_list = 0 -- Reset index
    ...
    current_focus_cycle_manager.current_idx_in_cycle_list = (current_focus_cycle_manager.current_idx_in_cycle_list % num_windows_in_cycle) + 1

    local window_id_to_focus = current_focus_cycle_manager.window_ids_in_order[current_focus_cycle_manager.current_idx_in_cycle_list]
    ...
    if window_to_focus and window_to_focus:isStandard() and not window_to_focus:isMinimized() then
        debug_log(string.format("Cycling to window %d of %d: %s (ID: %s)", current_focus_cycle_manager.current_idx_in_cycle_list,
        ...
        debug_log(string.format(
            ...
            window_id_to_focus, current_focus_cycle_manager.current_idx_in_cycle_list))
