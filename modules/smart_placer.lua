...
function smart_placer.find_best_tile(window, occupied_frames_override)
    ...
    -- Hoist window:application() + :name() into a single local (two AX calls → one pair).
    local win_app = window:application()
    local app_name = win_app and win_app:name()
    ...
    if not occupied_frames then
        ...
        if window_state_manager and window_state_manager.get and window_state_manager.get(window_id) then
            debug_log("Skipping smart placement, window already in a zone:", app_name)
            return nil
        end

        -- Read the occupied frames from the cache rather than enumerating every
        -- window via AX. get_for_screen_with_cache returns pre-filtered info
        -- entries (isStandard && !isMinimized && screen_id == ours), so we
        -- replace allWindows() + 4 per-window AX calls with one O(N) cache read.
        occupied_frames = {}
        local screen_id = screen:id()
        for _, info in ipairs(window_cache.get_for_screen_with_cache(screen_id)) do
            if info.window:id() ~= window_id then
                -- info.frame may be stale after a user-driven drag, but the smart
                -- placer already tolerates approximate overlap — it's a heuristic.
                table.insert(occupied_frames, info.frame)
            end
        end
    end
    ...
    -- Hoist screen:frame() out of the inner loop. screen:frame() is CoreGraphics
    -- (not AX), but it was being called ~4 times per candidate tile — for a
    -- layout with 10+ tiles this is dozens of redundant C calls per placement.
    local screen_frame = screen:frame()
    local screen_diag = math.sqrt(screen_frame.w ^ 2 + screen_frame.h ^ 2)
    local screen_area = screen_frame.w * screen_frame.h
        ...
        if zone_key ~= "default" then
            ...
            if zone_tiles then
                for i = 1, #zone_tiles do
                    ...
                    local closeness = 1.0 - (dist / screen_diag) -- 0 to 1
