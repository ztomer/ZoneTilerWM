...
local function adjust_grid(axis, index, delta)
    ...
    -- Iterate cached visible windows scoped to this screen to avoid an AX enumeration
    -- plus a per-window win:screen():id() round-trip (SentinelOne hooks those).
    for _, w in ipairs(window_cache.get_for_screen(screen:id())) do
...
local function handle_screen_change()
    ...
    -- Refresh window cache on screen change
    ...
    hs_timer.doAfter(config.tiler.delays.screen_change_reposition_sec, function()
        ...
        if config.tiler.reposition_on_screen_change then
            ...
            if reposition_callback then
                ...
                local all_windows = window_cache.get_all_visible()
...
function tiler.start()
    ...
    window_watcher = hs_window.filter.new()
    ...
    -- Initialize window cache for enterprise performance
    window_cache.init()
    -- Refresh after a short delay to ensure all windows are loaded
    hs_timer.doAfter(0.5, function()
        window_cache.refresh()
        local wins = window_cache.get_all_visible()
        debug_log("Window cache initialized with", #wins, "windows")
    end)

    -- Update cache on window events (async, doesn't block moves)
    window_watcher:subscribe({hs.window.filter.windowCreated, hs.window.filter.windowOpened}, function(win)
        window_cache.update(win, "created")
    end)
    window_watcher:subscribe(hs.window.filter.windowDestroyed, function(win)
