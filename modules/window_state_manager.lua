...
-- @param window_obj (hs.window|nil) Optional window object to avoid expensive lookup.
function window_state_manager.set(window_id, monitor_id, zone_key, tile_index, suppress_learning, window_obj)
    window_state.positions[window_id] = {
        monitor_id = monitor_id,
        zone_key = zone_key,
        tile_index = tile_index,
    }
    -- App memory update - use provided window object to avoid expensive lookup
    local window = window_obj or hs_window.get(window_id)
    if window then
