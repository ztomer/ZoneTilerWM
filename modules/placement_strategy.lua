...
function placement_strategy.find_best_tile(window, zone_key, zone_windows, zone, all_tiles_in_zone)
    ...
    debug_log('find_best_tile: START - window_id=' .. window:id() .. ' zone_key=' .. zone_key .. ' state_zone=' ..
                  (debug_state and debug_state.zone_key or 'nil') .. ' state_tile=' ..
                  (debug_state and debug_state.tile_index or 'nil'))
    ...
    elseif strategy == 'largest_free_space' then
        ...
        debug_log('largest_free_space: current zone_key=' .. (current_pos and current_pos.zone_key or 'nil') ..
                      ' target=' .. zone_key .. ' already_in_zone=' .. tostring(already_in_zone))
...
function placement_strategy.find_largest_free_tile(window, all_tiles_in_zone, already_in_zone)
    ...
    debug_log('Largest-free-space: checking ' .. #all_windows .. ' other windows, current at x=' .. current_frame.x ..
                  ' y=' .. current_frame.y)
    ...
    for i, v in ipairs(tile_options) do
        debug_log('  tile_options[' .. i .. ']: x=' .. v.tile.x .. ' y=' .. v.tile.y .. ' w=' .. v.tile.w .. ' h=' ..
                      v.tile.h .. ' available=' .. v.available)
    end

    -- Check if the best option is completely unusable (available <= 0)
    if tile_options[1].available <= 0 then
        debug_log('Largest-free-space strategy: All tiles fully blocked, falling back to rotation.')
        return placement_strategy.find_by_rotation(window, nil, {}, all_tiles_in_zone)
    end
    ...
    if already_in_zone and #tile_options > 1 then
        ...
        debug_log('largest_free_space: window_state has zone_key=' .. (state_pos and state_pos.zone_key or 'nil') ..
                      ' tile_index=' .. (state_pos and state_pos.tile_index or 'nil'))

        -- Also check tile_index_to_apply that was stored
        debug_log('largest_free_space: current_frame before comparison: x=' .. current_frame.x .. ' y=' ..
                      current_frame.y .. ' w=' .. current_frame.w .. ' h=' .. current_frame.h)
        ...
        for i, v in ipairs(tile_options) do
            ...
            debug_log('largest_free_space: compare tile ' .. i .. ' tile=(' .. v.tile.x .. ',' .. v.tile.y .. ',' ..
                          v.tile.w .. ',' .. v.tile.h .. ') frame=(' .. current_frame.x .. ',' .. current_frame.y .. ',' ..
                          current_frame.w .. ',' .. current_frame.h .. ') xdiff=' .. x_eq .. ' ydiff=' .. y_eq ..
                          ' wdiff=' .. w_eq .. ' hdiff=' .. h_eq .. ' match=' .. tostring(is_match))
            if is_match then
    ...
    debug_log('Largest-free-space strategy: Best tile (available=' .. tile_options[1].available .. ' out of ' ..
                  tile_options[1].area .. ')')
