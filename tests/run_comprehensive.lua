...
local corpus = require("tests.comprehensive_corpus")

-- Mocks
local mock_mm = { get_id = function() return "test_screen" end }
...
function MockWindow:frame() return {w=self._def.w, h=self._def.h} end
function MockWindow:screen() return { frame=function() return {x=0,y=0,w=1000,h=1000} end } end
function MockWindow:application() return { name = function() return self._def.id end } end
...
local function run_scenario(scen)
    ...
    if scen.expect.assignments then
        for app, expected_zone in pairs(scen.expect.assignments) do
            ...
            if type(expected_zone) == "table" then
                for _, z in ipairs(expected_zone) do if z == actual then match = true break end end
            else
            ...
            if not match then
                passed = false
                table.insert(reasons, "App " .. app .. " expected in " .. tostring(expected_zone) .. ", got " .. tostring(actual))
            end
        end
    end
    ...
    if passed then
        ...
        print("RESULT: FAIL")
        for _, r in ipairs(reasons) do print("  - " .. r) end
        return false
    end
end
...
for _, scen in ipairs(corpus.scenarios) do
    total = total + 1
    if run_scenario(scen) then passes = passes + 1 end
end
...
if passes < total then os.exit(1) else os.exit(0) end
