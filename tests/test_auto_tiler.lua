-- test_auto_tiler.lua
local function print_test(name)
    print('[TEST] ' .. name)
end
...
local mock_mm = {
    get_id = function(screen)
        return screen._id
    end,
}
local mock_wm = {
    get_remembered_position = function(app, mid)
        return nil
    end,
    get_ranked_preferences = function(app, mid)
        if app == 'AppA' then
            return { { zone_key = '1', tile_index = 1, count = 100 } }
        end
        return {}
    end,
}
local mock_zc = {
    ...
    get_layout_config = function()
        return { rows = 2, cols = 2 }, '2x2'
    end,
}
...
local mock_wa = {
    position_window_from_memory = function()
        return true
    end,
}
...
mock_wm.get_remembered_position = function(app, mid)
    if app == 'AppA' then
        return { zone_key = '1', tile_index = 1 }
    end
    return nil
end
...
mock_zc.get = function(mid, zk)
    if zk == '0' then
        return { { x = 250, y = 250, w = 500, h = 500 } }
    end -- Center tile
    if zk == 'j' then
        return { { x = 250, y = 250, w = 500, h = 500 } }
    end -- Fallback Center
...
for _, m in ipairs(moved_windows) do
    if m.id == 3 then
        if m.zk == '0' or m.zk == 'j' then
            foundC = true
            print('PASS: AppC (Focused) should go to zone 0 or j (Center)')
        else
            print('FAIL: AppC (Focused) should go to zone 0 or j (Center) Expected 0 or j, got ' .. tostring(m.zk))
            os.exit(1)
        end
    end
end
...
mock_zc.get = function(mid, zk)
    ...
    if zk == 'j' then
        return {
            { x = 450, y = 450, w = 100, h = 100 }, -- Tile 1: 10k area, covers 500,500
        }
    end
    return nil
end
