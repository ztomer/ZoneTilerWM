...
-- Mocks
local mock_mm = {
    get_id = function(s) return "screen1" end
}

local mock_zc = {
    get_layout_config = function(s) return {}, "test_layout" end,
    get = function(mid, zone_key)
        if zone_key == "ZoneA" then return {{x=0,y=0,w=100,h=200}, {x=0,y=200,w=100,h=200}} end -- Tall
        if zone_key == "ZoneB" then return {{x=100,y=0,w=200,h=100}} end -- Wide
        return {}
    end
}

local mock_wm = {
    get_ranked_preferences = function() return {} end,
...
local function make_window(app_name, w, h)
    return {
        ...
        frame = function() return {x=0, y=0, w=w, h=h} end,
        screen = function()
             return {
                 frame = function() return {x=0,y=0,w=1000,h=1000} end,
                 name = function() return "Screen 1" end,
                 id = function() return "screen1" end
             }
        end,
...
local function test_solver_integration()
    ...
    mock_hs.window.allWindows = function() return {wWide, wTall} end
    mock_hs.screen.allScreens = function() return { wWide:screen() } end
    mock_hs.window.focusedWindow = function() return nil end -- No focused window for this test
