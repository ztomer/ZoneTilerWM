...
local function setup_env()
    ...
    auto_tiler.init(
        config,
        { set_reposition_callback = function() end },
        window_memory,
        smart_placer,
        zone_calculator,
        monitor_manager,
...
local function create_win(id, app_name, focused, frame)
    local win = {
        ...
        screen = function()
            return mock_hs.screen.allScreens()[1]
        end,
        frame = function()
            return frame or { x = 0, y = 0, w = 100, h = 100 }
        end,
...
local function test_ripple()
    ...
    for _, f in ipairs(frames) do
        if f then
            if math.abs(f.y - y1.y) < 1 then
                has_y1 = true
            end
            if math.abs(f.y - y2.y) < 1 then
                has_y2 = true
            end
        end
    end
...
local function test_smart_placement()
    ...
    window_memory.get_ranked_preferences = function()
        return {}
    end
